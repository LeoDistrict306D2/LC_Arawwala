import type { Metadata } from 'next';
import { notFound } from 'next/navigation';
import Link from 'next/link';
import { club } from '@/content/club';
import { projects } from '@/content/projects';
import { formatDate } from '@/lib/utils';
import { PageMasthead } from '@/components/PageMasthead';
import { Photo } from '@/components/Photo';

/** Static params from content, so every entry is prerendered at build time. */
export function generateStaticParams() {
  return projects.map((project) => ({ slug: project.slug }));
}

type Params = { params: Promise<{ slug: string }> };

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { slug } = await params;
  const project = projects.find((entry) => entry.slug === slug);
  if (!project) return {};

  return {
    title: project.title,
    description: project.summary,
    alternates: { canonical: `/projects/${project.slug}` },
    openGraph: {
      type: 'article',
      title: `${project.title} — ${club.name}`,
      description: project.summary,
      publishedTime: project.date,
      images: [
        {
          url: project.heroImage.src,
          width: project.heroImage.width,
          height: project.heroImage.height,
          alt: project.heroImage.alt,
        },
      ],
    },
  };
}

export default async function ProjectPage({ params }: Params) {
  const { slug } = await params;
  const project = projects.find((entry) => entry.slug === slug);
  if (!project) notFound();

  const entryNumber = project.id.replace(/^\D*/, '').padStart(3, '0');

  // Same category first, then anything else, so "related" is never empty.
  const related = projects
    .filter((entry) => entry.slug !== project.slug)
    .sort((a, b) => {
      const aMatch = a.category === project.category ? 0 : 1;
      const bMatch = b.category === project.category ? 0 : 1;
      return aMatch - bMatch || b.date.localeCompare(a.date);
    })
    .slice(0, 3);

  return (
    <>
      <PageMasthead
        kicker={`Entry ${entryNumber}`}
        title={project.title}
        standfirst={project.summary}
        breadcrumb={{ href: '/projects', label: 'All entries' }}
      />

      <div className="wrap pt-10">
        <Photo image={project.heroImage} ratio="wide" priority sizes="100vw" />
      </div>

      <div className="wrap band grid gap-12 md:grid-cols-12">
        <div className="md:col-span-7">
          {project.story && project.story.length > 0 ? (
            project.story.map((paragraph, index) => (
              <p
                key={index}
                className="measure mb-6 text-lg leading-relaxed text-ink-muted last:mb-0"
              >
                {paragraph}
              </p>
            ))
          ) : (
            <p className="measure text-lg leading-relaxed text-ink-muted">
              {project.summary}
            </p>
          )}

          {project.objectives && project.objectives.length > 0 ? (
            <section className="mt-12" aria-labelledby="objectives">
              <h2
                id="objectives"
                className="text-[0.7rem] uppercase tracking-[0.22em] text-accent"
              >
                What we set out to do
              </h2>
              <ul className="mt-4 border-t border-rule">
                {project.objectives.map((objective) => (
                  <li
                    key={objective}
                    className="border-b border-rule py-3 text-ink-muted"
                  >
                    {objective}
                  </li>
                ))}
              </ul>
            </section>
          ) : null}
        </div>

        <aside className="md:col-span-5">
          <dl className="border-t-2 border-ink">
            {[
              { term: 'Date', value: formatDate(project.date) },
              { term: 'Leo year', value: project.year },
              { term: 'Location', value: project.location },
              { term: 'Category', value: project.category.replace(/-/g, ' ') },
            ]
              .filter((row) => Boolean(row.value))
              .map((row) => (
                <div key={row.term} className="flex justify-between gap-6 border-b border-rule py-3">
                  <dt className="text-xs uppercase tracking-[0.16em] text-ink-faint">{row.term}</dt>
                  <dd className="text-right text-sm text-ink capitalize">{row.value}</dd>
                </div>
              ))}
          </dl>

          {project.impact && project.impact.length > 0 ? (
            <section className="mt-10" aria-labelledby="impact">
              <h2 id="impact" className="text-[0.7rem] uppercase tracking-[0.22em] text-accent">
                Measured
              </h2>
              <dl className="mt-4">
                {project.impact.map((stat) => (
                  <div
                    key={stat.id}
                    className="flex items-baseline justify-between gap-6 border-b border-rule py-3"
                  >
                    <dt className="text-sm text-ink-muted">{stat.label}</dt>
                    <dd className="font-serif text-2xl font-semibold tabular-nums text-ink">
                      {stat.prefix}
                      {typeof stat.value === 'number'
                        ? stat.value.toLocaleString('en-LK')
                        : stat.value}
                      {stat.suffix}
                    </dd>
                  </div>
                ))}
              </dl>
            </section>
          ) : null}

          {project.partners && project.partners.length > 0 ? (
            <section className="mt-10" aria-labelledby="partners">
              <h2 id="partners" className="text-[0.7rem] uppercase tracking-[0.22em] text-accent">
                With
              </h2>
              <ul className="mt-4 space-y-2">
                {project.partners.map((partner) => (
                  <li key={partner.name} className="text-sm text-ink-muted">
                    {partner.url ? (
                      <a
                        href={partner.url}
                        target="_blank"
                        rel="noreferrer noopener"
                        className="underline underline-offset-2 hover:text-accent"
                      >
                        {partner.name}
                      </a>
                    ) : (
                      partner.name
                    )}
                  </li>
                ))}
              </ul>
            </section>
          ) : null}
        </aside>
      </div>

      {project.gallery && project.gallery.length > 0 ? (
        <section className="wrap pb-16" aria-labelledby="entry-gallery">
          <h2 id="entry-gallery" className="text-[0.7rem] uppercase tracking-[0.22em] text-accent">
            From the day
          </h2>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {project.gallery.map((image) => (
              <Photo
                key={image.src}
                image={image}
                ratio="landscape"
                sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw"
              />
            ))}
          </div>
        </section>
      ) : null}

      {related.length > 0 ? (
        <section className="border-t border-rule" aria-labelledby="related">
          <div className="wrap band">
            <h2 id="related" className="text-[0.7rem] uppercase tracking-[0.22em] text-accent">
              Nearby entries
            </h2>
            <ul className="mt-6">
              {related.map((entry) => (
                <li key={entry.id} className="border-b border-rule first:border-t">
                  <Link
                    href={`/projects/${entry.slug}`}
                    className="flex flex-col gap-1 py-5 transition-colors hover:text-accent sm:flex-row sm:items-baseline sm:justify-between sm:gap-6"
                  >
                    <span className="font-serif text-xl font-semibold">{entry.title}</span>
                    <span className="shrink-0 text-xs uppercase tracking-[0.16em] text-ink-faint">
                      {formatDate(entry.date, { year: 'numeric', month: 'short' })}
                    </span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </section>
      ) : null}
    </>
  );
}
